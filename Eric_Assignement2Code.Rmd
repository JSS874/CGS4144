...
mapped_df <- getBM(
  ...
  values = expression_df$Ensembl, # Replace 'Gene' with your Ensembl ID column name
  mart = mart
)
...
final_mapped_df <- collapsed_mapped_df %>%
  inner_join(expression_df, by = c("Ensembl" = "Ensembl"))
...
Looks like we have some cases where 3 gene symbols mapped to a single Ensembl ID. We have a total of 130 out of 17984 Ensembl IDs with multiple mappings to gene symbols.
...
results <- file.path(results_dir, "ERP105973_Gene_Symbols.tsv")

# Install packages
#install.packages("ggplot2")

data <- read.table(results, header = TRUE)
# Remove specified columns using base R
data <- data[, !names(data) %in% c("Ensembl", "all_entrez_ids", "first_mapped_entrez_id")]
...
my_data <- as.matrix(data[sapply(data, is.numeric)])
my_data <- round(my_data)
samples <- colnames(my_data)

meta_data$question <- ifelse(meta_data$refinebio_treatment == "enriched", "enriched", "barren")


#library(ggplot2)  # For plotting
install.packages("BiocManager")
BiocManager::install("DESeq2")
library(DESeq2)   # For PCA functions
...
new_data <- assay(rld)
t(new_data)
pc_object <- prcomp(new_data)

pc_x <- pc_object$PC1
pc_y <- pc_object$PC2

pca_data <- as.data.frame(pc_object$rotation[, c("PC1", "PC2")])
pca_data$Group <- meta_data$refinebio_treatment
...
ggplot(pca_data, aes(x = PC1, y = PC2, color = Group)) +
  geom_point(size = 3) +
  labs(
    title = "Frontal Cortex Functioning of Barren vs. Enriched Piglets PCA Plot",
    x = "Principal Component 1",
    y = "Principal Component 2"
  ) +
  theme_minimal() +
  theme(legend.title = element_text(size = 12), legend.text = element_text(size = 10)) +
  scale_color_manual(values = c("enriched" = "red", "barren" = "blue"))  # Adjust colors as needed
